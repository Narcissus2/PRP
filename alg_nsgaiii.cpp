...
void CNSGAIII::Solve(CPopulation *solutions, const BProblem &problem)
{
	...
	for (size_t t=0; t<gen_num_; t+=1)
	{
		...
		double Max_dis = pop[cur][0].objs()[0], Min_dis = pop[cur][0].objs()[0],Max_emi = pop[cur][0].objs()[1],Min_emi = pop[cur][0].objs()[1];
		for (size_t p = 0; p < PopSize; p++)
		{
			...
			bool same = false;
			if (pop[cur][p].objs()[0] > Max_dis)
			{
				Max_dis = pop[cur][p].objs()[0];
			}
			if (pop[cur][p].objs()[0] < Min_dis)
			{
				Min_dis = pop[cur][p].objs()[0];
			}
		...
		genfile << "Max dis = " << Max_dis << endl;
		genfile << "Min dis = " << Min_dis << endl;
		genfile << "Max emi = " << Max_emi << endl;
		genfile << "Min emi = " << Min_emi << endl;
		if (Min_dis < G_Min_dis)
		{
			genfile << t + 1 << "代更新最佳 " << Min_dis << endl;
			G_Min_dis = Min_dis;
			//cout << t + 1 << "代更新最佳 " << Min_dis << endl;
		}
		double normal_dis = Max_dis - Min_dis, normal_emi = Max_emi - Min_emi;
		...
		for (size_t i = 0; i < PopSize; i += 2)
		{
			...
			PNN2O(&pop[cur][PopSize + i], problem, 100 - father, normal_dis, normal_emi);
			...
			PNN2O(&pop[cur][PopSize + i + 1], problem, 100 - mother, normal_dis, normal_emi);
			...
			//if(t >=33 && t < 66){
				...
				PNEH2O(&pop[cur][PopSize + i], problem,100-father, normal_dis, normal_emi);
			...
			//if (t >= 33 && t < 66) {
				...
				PNEH2O(&pop[cur][PopSize + i + 1], problem,100-mother, normal_dis, normal_emi);
			...
			//if(t >= 66){
				...
				OPT2O(&pop[cur][PopSize + i], problem, 100 - father, normal_dis, normal_emi, Min_dis, Min_emi);
				...
				//OPT2O(&pop[cur][PopSize + i], problem, 100, normal_dis, normal_emi, Min_dis, Min_emi);
			}
			if (num2 == OPT2_index) {
			//if (t >= 66) {
				//	OPT2(&pop[cur][PopSize + i + 1], problem);
				OPT2O(&pop[cur][PopSize + i + 1], problem, 100 - mother, normal_dis, normal_emi, Min_dis, Min_emi);
				...
				//OPT2O(&pop[cur][PopSize + i + 1], problem, 100, normal_dis, normal_emi, Min_dis, Min_emi);
			}
			...
			cnt += problem.Dp2Object(&pop[cur][PopSize + i],100 - father);
			cnt += problem.Dp2Object(&pop[cur][PopSize + i + 1],100 - mother);
			total_evaluate += 2;

			//cnt += problem.Dp2Object(&pop[cur][PopSize + i], 100);
			//cnt += problem.Dp2Object(&pop[cur][PopSize + i + 1], 100);
			//system("pause");
			if (pop[cur][PopSize + i].objs()[0]-5 <= Min_dis && t<0)
			{
				APO(&pop[cur][PopSize + i], problem, 100 - father, normal_dis, normal_emi, Min_dis, Min_emi);
				RAP(&pop[cur][PopSize + i], problem);
				problem.Dp2Object(&pop[cur][PopSize + i], 100 - father);
				total_evaluate += pop[cur][PopSize + i].num_evaluate()+1;
			}
			if (pop[cur][PopSize + i + 1].objs()[0]-5 <= Min_dis && t<0)
			{
				APO(&pop[cur][PopSize + i + 1], problem, 100 - mother, normal_dis, normal_emi, Min_dis, Min_emi);
				RAP(&pop[cur][PopSize + i + 1], problem);
				problem.Dp2Object(&pop[cur][PopSize + i + 1], 100 - mother);
				total_evaluate += pop[cur][PopSize + i + 1].num_evaluate()+1;
			}
...
void CNSGAIII::CheckSame_pop(const CPopulation p)
{
	bool flag = false;
